...
Token.prototype.refresh = (function () {
	...
	return function () {
		...
		//get the border size
		let borderSize =
			this.data?.tempHexValues?.borderSize ||
			this.document.getFlag("hex-size-support", "borderSize");

		let alwaysShowBorder = game.settings.get("hex-size-support", "alwaysShowBorder");
		...
		if (alwaysShowBorder == true || borderSize != undefined /*&& borderSize != 1*/) {
			...
			if (!!borderColor) {
				...
				let borderWidth = game.settings.get("hex-size-support", "borderWidth");

				this.border.clear();
				this.border.alpha = 1;
				this.border.lineStyle(borderWidth + 2, 0x000000, 0.8).drawPolygon(shiftedPoints.flat());
				this.border
					.lineStyle(borderWidth, borderColor || 0xff9829, 1.0)
					.drawPolygon(shiftedPoints.flat());
